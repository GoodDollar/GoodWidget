...
>
  ...
  >
    ...
    <DocsCallout title="On-chain (not backend)" tone="info">
      Operator consent, vault minimum reads, quotes, withdrawable principal, and Celo deposit/stream
      transactions are handled on-chain via RPC. There is no channel-list API — paste the channel ID
      manually.
    </DocsCallout>
  </DocsSection>
  ...
  >
    ...
    <DocsCallout title="Base bridge" tone="info">
      Close, withdraw, and credit funding require the worker&apos;s Base operator bridge to be
      configured (`bridge.enabled: true` in API responses).
    </DocsCallout>
  </DocsSection>
