/**
 * App — AntSeed AI Credits analytics dashboard. Read-only data visualization,
 * no wallet connection of any kind. Ported from GoodDollar/data-team's
 * reference dashboard (projects/antseed-analytics/dashboard/app.js), which
 * renders the same 3 scorecards + 3 charts + paginated table via vanilla
 * JS/Chart.js/DOM, using @goodwidget/ui's chart components instead.
 */
...
import { flowRateToDaily, weiToGd, weiToUsd } from './lib/analyticsConversions'
import type { DailyAnalyticsRecord } from './lib/analyticsApi'
...
function DashboardHeader({
  ...
  source: 'live' | 'demo'
  onSelectSource: (source: 'live' | 'demo') => void
  ...
  return (
    <TamaguiProvider config={defaultConfig} defaultTheme="dark">
      <YStack
        ...
        alignItems="center"
        data-testid="ai-credits-analytics-page"
      >
        <YStack width="100%" maxWidth={CONTENT_MAX_WIDTH} paddingHorizontal="$6" paddingVertical="$8" $sm={{ paddingHorizontal: '$4', paddingVertical: '$6' }}>
          <DashboardContent />
        </YStack>
      </YStack>
    </TamaguiProvider>
