...
/**
 * Local copy of the superfluid widget's chip. Kept package-local on purpose: promoting
 * it to @goodwidget/ui would pull that widget into this change. Worth revisiting the
 * next time a third widget needs a wallet chip.
 */
...
export function WalletChip({
  ...
  return (
    <XStack position="relative" alignItems="center" opacity={disabled ? 0.5 : 1}>
      ...
      >
        <YStack width={6} height={6} borderRadius="$full" backgroundColor="$success" />
        <Text fontSize="$1" numberOfLines={1}>
          {address ? shortAddress(address) : ''}
        </Text>
        <Icon name="chevron-down" size="2xs" color="muted" />
      </XStack>
        ...
        <>
          ...
          >
            ...
            >
              ...
              <ButtonText fontSize="$2">{disconnectLabel}</ButtonText>
            </Button>
            {disconnectMessage && (
